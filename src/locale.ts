...
export class Locale {
  ...
  /**
   * Format a number for this locale using a specified {@link NumberPattern}
   * @param {Numeric|String|Decimal} number number to format
   * @param {NumberPattern} pattern number pattern
  ...
  formatNumber(
    ...
    return format(number, pattern, { locale: this, ...rest });
  }
   ...
   */
  formatDecimal(number: Decimal | number | string, quantize = true): string {
    if (!this.decimalPattern) {
      return new Decimal(number).toString();
    }
    return this.formatNumber(number, this.decimalPattern, { quantize });
  }
  ...
  formatCurrency(
    ...
    if (formatType === 'standard' && this.currencyPattern) {
      pattern = this.currencyPattern;
    } else if (formatType === 'accounting' && this.accountingPattern) {
      pattern = this.accountingPattern;
    } else {
      throw new Error(
    ...
    return this.formatNumber(number, pattern, {
      currency,
      currencyDigits,
      quantize,
    });
