/* eslint-disable camelcase, unicorn/prevent-abbreviations */
/**
 * Automatically generated by gen_l10n_data.py
 * @packageDocumentation
 * @internal
 */
import { Locale } from '../locale';
import root from './root';

// Unicode CLDR Version 37, retrieved 2020-10-13
export const ca = new Locale(
  'ca',
  {
    d: ',',
    g: '.',
    cs: { AUD: 'AU$', JPY: '¥', THB: '฿' },
    cn: {
      AED: 'dírham de la UEA',
      AFN: 'afgani afganès',
      ALL: 'lek albanès',
      AMD: 'dram armeni',
      ANG: 'florí de les Antilles Neerlandeses',
      AOA: 'kwanza angolès',
      ARS: 'peso argentí',
      AUD: 'dòlar australià',
      AWG: 'florí d’Aruba',
      AZN: 'manat azerbaidjanès',
      BAM: 'marc convertible de Bòsnia i Hercegovina',
      BBD: 'dòlar de Barbados',
      BDT: 'taka de Bangla Desh',
      BGN: 'lev búlgar',
      BHD: 'dinar de Bahrain',
      BIF: 'franc de Burundi',
      BMD: 'dòlar de les Bermudes',
      BND: 'dòlar de Brunei',
      BOB: 'bolivià',
      BRL: 'real brasiler',
      BSD: 'dòlar de les Bahames',
      BTN: 'ngultrum de Bhutan',
      BWP: 'pula de Botswana',
      BYN: 'ruble bielorús',
      BZD: 'dòlar de Belize',
      CAD: 'dòlar canadenc',
      CDF: 'franc congolès',
      CHF: 'franc suís',
      CLP: 'peso xilè',
      CNY: 'iuan xinès',
      COP: 'peso colombià',
      CRC: 'colon costa-riqueny',
      CUC: 'peso convertible cubà',
      CUP: 'peso cubà',
      CVE: 'escut de Cap Verd',
      CZK: 'corona txeca',
      DJF: 'franc de Djibouti',
      DKK: 'corona danesa',
      DOP: 'peso dominicà',
      DZD: 'dinar algerià',
      EGP: 'lliura egípcia',
      ERN: 'nakfa eritreu',
      ETB: 'birr etíop',
      EUR: 'euro',
      FJD: 'dòlar fijià',
      FKP: 'lliura de les illes Malvines',
      GBP: 'lliura esterlina britànica',
      GEL: 'lari georgià',
      GHS: 'cedi ghanès',
      GIP: 'lliura de Gibraltar',
      GMD: 'dalasi gambià',
      GNF: 'franc guineà',
      GTQ: 'quetzal guatemalenc',
      GYD: 'dòlar de Guyana',
      HKD: 'dòlar de Hong Kong',
      HNL: 'lempira hondurenya',
      HRK: 'kuna croata',
      HTG: 'gourde haitià',
      HUF: 'fòrint hongarès',
      IDR: 'rupia indonèsia',
      ILS: 'nou xéquel israelià',
      INR: 'rupia índia',
      IQD: 'dinar iraquià',
      IRR: 'rial iranià',
      ISK: 'corona islandesa',
      JMD: 'dòlar jamaicà',
      JOD: 'dinar jordà',
      JPY: 'ien japonès',
      KES: 'xíling kenyà',
      KGS: 'som kirguís',
      KHR: 'riel cambodjà',
      KMF: 'franc de les Comores',
      KPW: 'won nord-coreà',
      KRW: 'won sud-coreà',
      KWD: 'dinar kuwaitià',
      KYD: 'dòlar de les illes Caiman',
      KZT: 'tenge kazakh',
      LAK: 'kip laosià',
      LBP: 'lliura libanesa',
      LKR: 'rupia de Sri Lanka',
      LRD: 'dòlar liberià',
      LSL: 'loti de Lesotho',
      LYD: 'dinar libi',
      MAD: 'dírham marroquí',
      MDL: 'leu moldau',
      MGA: 'ariary malgaix',
      MKD: 'denar macedoni',
      MMK: 'kyat de Myanmar',
      MNT: 'tögrög mongol',
      MOP: 'pataca de Macau',
      MRU: 'ouguiya maurità',
      MUR: 'rupia mauriciana',
      MVR: 'rupia de les Maldives',
      MWK: 'kwacha malawià',
      MXN: 'peso mexicà',
      MYR: 'ringgit de Malàisia',
      MZN: 'metical moçambiquès',
      NAD: 'dòlar namibià',
      NGN: 'naira nigerià',
      NIO: 'córdoba nicaragüenc',
      NOK: 'corona noruega',
      NPR: 'rupia nepalesa',
      NZD: 'dòlar neozelandès',
      OMR: 'rial omanita',
      PAB: 'balboa panameny',
      PEN: 'sol peruà',
      PGK: 'kina de Papua Nova Guinea',
      PHP: 'peso filipí',
      PKR: 'rupia pakistanesa',
      PLN: 'zloty polonès',
      PYG: 'guaraní paraguaià',
      QAR: 'rial de Qatar',
      RON: 'leu romanès',
      RSD: 'dinar serbi',
      RUB: 'ruble rus',
      RWF: 'franc de Ruanda',
      SAR: 'rial saudita',
      SBD: 'dòlar de les illes Salomó',
      SCR: 'rupia de les Seychelles',
      SDG: 'lliura sudanesa',
      SEK: 'corona sueca',
      SGD: 'dòlar de Singapur',
      SHP: 'lliura de Santa Helena',
      SLL: 'leone de Sierra Leone',
      SOS: 'xíling somali',
      SRD: 'dòlar de Surinam',
      STN: 'dobra de São Tomé i Príncipe',
      SVC: 'colon salvadorenc',
      SYP: 'lliura siriana',
      SZL: 'lilangeni swazi',
      THB: 'baht tailandès',
      TJS: 'somoni tadjik',
      TMT: 'manat turcman',
      TND: 'dinar tunisià',
      TOP: 'pa‘anga tongà',
      TRY: 'lira turca',
      TTD: 'dòlar de Trinitat i Tobago',
      TWD: 'nou dòlar de Taiwan',
      TZS: 'xíling tanzà',
      UAH: 'hrívnia ucraïnesa',
      UGX: 'xíling ugandès',
      USD: 'dòlar dels Estats Units',
      UYU: 'peso uruguaià',
      UZS: 'som uzbek',
      VND: 'dong vietnamita',
      VUV: 'vatu de Vanuatu',
      WST: 'tala samoà',
      XAF: 'franc CFA BEAC',
      XCD: 'dòlar del Carib Oriental',
      XDR: 'drets especials de gir',
      XOF: 'franc CFA BCEAO',
      XPF: 'franc CFP',
      XXX: 'moneda desconeguda',
      YER: 'rial iemenita',
      ZAR: 'rand sud-africà',
      ZMW: 'kwacha zambià',
    },
    cp: '#,##0.00 ¤',
    ap: '#,##0.00 ¤;(#,##0.00 ¤)',
  },
  root
);

// Locale Data
export const ca_AD = new Locale('ca_AD', { c: 'EUR', cs: {}, cn: {} }, ca);
export const ca_ES_VALENCIA = new Locale(
  'ca_ES_VALENCIA',
  { c: 'EUR', cs: {}, cn: {} },
  ca
);
export const ca_FR = new Locale('ca_FR', { c: 'EUR', cs: {}, cn: {} }, ca);
export const ca_IT = new Locale('ca_IT', { c: 'EUR', cs: {}, cn: {} }, ca);
export const ca_ES = new Locale('ca_ES', { c: 'EUR' }, ca);

// Default Export
export default ca;
